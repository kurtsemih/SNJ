...
import copy
import argparse
from snj.utils import convert_data_str_to_onehot, newick2nx
...
def orient_pick(held_out, leaves_at_adj, num_of_ort):
    ...
    # if less leaves than the number of orienting leaves
    ...
    # if the number of leaves equals to the number of orienting leaves
    elif num_leaf_at_adj == num_of_ort:
        ort_leaves = list(leaves_at_adj.copy())
    # if more leaves than the number of orienting leaves
    else:
        ...
        computed_distances[held_out][leaves_at_adj_samp] = dist_vector.copy()
        computed_distances[leaves_at_adj_samp][:, held_out] = dist_vector.T.copy()
...
def decomposeTree(root, tree, held_out, current_degree=0, first_placement=False, subtree_leaves=None,
    ...
    for k in range(num_of_ort):
        for l in range(1, 4):
            for m in range(l):
                if computed_distances[int(orients[k, l])][int(orients[k, m])] == -1:
                    ...
                    computed_distances[int(orients[k, l])][int(orients[k, m])] = dist_matrix[k, l, m]
                else:
...
def main():
    ...
    computed_distances = -1*np.ones([n_leaves, n_leaves])
    computed_distances[range(n_leaves), range(n_leaves)] = 0
    ...
    for seed in range(num_of_seed):
        ...
        for ki in range(initial_n_leaves):
            dist_matrix[ki, ki+1:] = np.abs(data_onehot_initial[ki+1:, :, :] - data_onehot_initial[ki, :, :]).sum(axis=(-1,-2))

        dist_matrix += dist_matrix.T
        computed_distances[initial_leaves][:, initial_leaves] = dist_matrix.copy()
        dist_matrix = dist_matrix / (2 * seq_length)
        dist_matrix = (-3 / 4) * np.log(1 - dist_matrix * 4 / 3)
